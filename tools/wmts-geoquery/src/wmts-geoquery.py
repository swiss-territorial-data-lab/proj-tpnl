...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser( description = "Image query specific module - SWISSIMAGE swisstopo" )

    # Define argument and their parameter
    parser.add_argument( '--url'  , type=str, help = 'WMTS capabilities URL' )
    parser.add_argument( '--layer', type=str, help = 'WMTS layer identifier' )
    parser.add_argument( '--tiles', type=str, help = 'Tiles definition geographical file' )
    parser.add_argument( '--width', type=int, help = 'Tile GeoTiff width, in pixels' )
    parser.add_argument( '--time' , type=str, help = 'Layer time, when available' )
    parser.add_argument( '--tmp'  , type=str, help = 'Temporary directory to use', default = '/run/shm' )
    ...
    service = requests.get( args.url, allow_redirects=True, verify=False )
    ...
    layer = detect_layer( service_xml_path, service_xml_data, args.layer )
    ...
    if query_url.find( '{Time}' ) != -1:
        query_url = query_url.replace( '{Time}', args.time )
    ...
    vector_tile = gpd.read_file( args.tiles ).to_crs( 'EPSG:2056' )
    ...
    for index, tile in vector_tile.iterrows():
        ...
        tile_export = f"{math.floor(bounding_box[0])}-{math.floor(bounding_box[1])}-{math.floor(tile_geo_w)}-{math.floor(tile_geo_h)}-{tile_pixel_w}-{tile_pixel_h}.tif"
